"use client";
// app/page.tsx — Javari Fitness · CR AudioViz AI · EIN 39-3646201
import { useState } from "react";
const TOOLS = [{"icon": "\ud83c\udfcb\ufe0f", "label": "Workout Plan", "desc": "Custom training programs", "href": "/workout"}, {"icon": "\ud83e\udd57", "label": "Nutrition Guide", "desc": "Meal plans and macros", "href": "/nutrition"}, {"icon": "\ud83d\udce7", "label": "Client Check-in", "desc": "Progress tracking emails", "href": "/checkin"}, {"icon": "\ud83d\udcf1", "label": "Social Content", "desc": "Fitness motivation content", "href": "/social"}];
export default function Page() {
  ...
  async function go() { if(!input.trim())return; setLoading(true);setOut("");
    try { const r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:[{role:"user",content:input}],stream:false,systemOverride:"You are a certified personal trainer and nutrition coach. Create safe, effective fitness programs. Not a substitute for medical advice."})});
  ...
  return (<div style={{minHeight:"100vh",background:"#040912",color:"#e2e8f0",fontFamily:"system-ui"}}>
    <nav style={{background:"#1E3A5F",padding:"0 20px",height:52,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:20}}>💪</span><span style={{fontWeight:800,color:"#FF0800",fontSize:15}}>Javari Fitness</span></div>
    ...
    <section style={{background:"linear-gradient(135deg,#1E3A5F,#040912)",padding:"56px 24px 48px",textAlign:"center"}}>
      <h1 style={{fontSize:"clamp(24px,4vw,44px)",fontWeight:900,color:"#fff",margin:"0 0 12px",lineHeight:1.05}}>AI Fitness<br/><span style={{color:"#FF0800"}}>Coaching Tools</span></h1>
      <p style={{color:"rgba(255,255,255,0.7)",fontSize:15,lineHeight:1.65,margin:0}}>Custom workout plans, nutrition guides, client programs, and gym marketing.</p>
    </section>
    <section style={{maxWidth:720,margin:"0 auto",padding:"28px 20px 0"}}>
      <div style={{background:"#0F1F32",border:"1px solid rgba(0,180,216,0.12)",borderRadius:14,padding:"20px 24px"}}>
        <h2 style={{margin:"0 0 12px",fontSize:15,fontWeight:700,color:"#fff"}}>Build Workout Plan</h2>
        <div style={{display:"flex",gap:8}}><input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Client: 35F, goal: lose 20lbs, 4 days/week, home gym with dumbbells..." style={{flex:1,background:"#172D48",border:"1px solid rgba(0,180,216,0.15)",borderRadius:8,padding:"10px 12px",color:"#e2e8f0",fontSize:13,outline:"none",fontFamily:"system-ui"}}/>
